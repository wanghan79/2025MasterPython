...
def initialize_exp(params):
    ...
    json.dump(vars(params), open(os.path.join(exp_folder, time_now+'params.pkl'), 'w'), indent=4)
    ...
    # create a logger
    time_now = datetime.now().strftime("%H:%M")
    logger = create_logger(os.path.join(exp_folder,time_now+'_' 'train.log'), rank=getattr(params, 'global_rank', 0))

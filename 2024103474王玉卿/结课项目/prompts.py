...
def build_verification_decomp_prompt(e1: Optional[Event], e2: Optional[Event])-> Dict[str, str]:
    ...
    start_prompt = f'Does "{d1}" start before "{d2}"? Answer with only one word: yes, no, or unknown'
    end_prompt = f'Does "{d1}" end before "{d2}"? Answer with only one word: yes, no, or unknown'
    duration_prompt = f'Is the duration of "{d1}"longer than "{d2}"? Answer with only one word: yes, no, or unknown'
...
def build_verification_prompt(e1:Optional[Event], e2: Optional[Event])-> Dict[str, Dict]:
    ...
    temporal_prompt = f'Does "{d1}" happen before or after "{d2}"? Answer with only one word: yes, no, or unknown'
    hierarchy_prompt = f'Is "{d2}" a part of "{d1}"? Answer with only one word: yes, no, or unknown'

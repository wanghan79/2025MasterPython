...
class DataSampler:
    ...
    def _generate_int(self, min_value: int = -100, max_value: int = 100) -> int:
        return random.randint(min_value, max_value)

    def _generate_float(self, min_value: float = -100.0, max_value: float = 100.0) -> float:
        return random.uniform(min_value, max_value)

    def _generate_str(self, str_length: int = 10) -> str:
    ...
    def _generate_list(self, item_schema: Dict[str, Any], list_length: int = 3) -> List[Any]:
        return [self.generate_sample(item_schema) for _ in range(list_length)]

    def _generate_tuple(self, item_schema: Dict[str, Any], tuple_length: int = 3) -> Tuple[Any, ...]:
        return tuple(self.generate_sample(item_schema) for _ in range(tuple_length))

    def _generate_dict(self, dict_schema: Dict[str, Any]) -> Dict[str, Any]:
        return {key: self.generate_sample(value) for key, value in dict_schema.items()}
    ...
    def generate_samples(self, schema: Dict[str, Any], sample_count: int = 1) -> List[Any]:
        return [self.generate_sample(schema) for _ in range(sample_count)]
...
if __name__ == "__main__":
    ...
    # 示例4：生成多个样本
    print("\nMultiple samples:", sampler.generate_samples(user_schema, sample_count=2))

...
sys.path.append("/mnt/Data6/hjy/STEP")
...
class EsmPPIModel(pl.LightningModule):
    ...
    def __init__(self, params) -> None:
        ...
        self.model_name = "/mnt/Data6/hjy/STEP/esm2/esm2_t12_35M_UR50D/"
    ...
    def __build_model(self) -> None:
        ...
        self.original_esm_dim = 480  # ESM-2 150M模型的隐藏层维度
        self.adapted_dim = 512     

        self.dim_adapter = nn.Sequential(
            nn.Linear(self.original_esm_dim * 4, self.adapted_dim * 4),  # 1920->2048
            nn.GELU(),
            nn.LayerNorm(self.adapted_dim * 4)
        )
        ...
        self.classification_head = nn.Sequential(OrderedDict([
            ...
            ("dense1", nn.Linear(self.adapted_dim * 4, int(self.adapted_dim * 4 / 8))),  # 2048->128
            ("dropout2", nn.Dropout(0.2)),
            ("dense2", nn.Linear(int(self.adapted_dim * 4 / 8), int(self.adapted_dim * 4 / (16*16)))),  # 128->8
    ...
    def pool_strategy(self, features):
        ...
        # 四种池化方式（与原始ProtBert保持一致）
        pool_vectors = []
    
        # 1. CLS Token池化
        pool_vectors.append(features['cls_token_embeddings'])  # [batch, 480]
    
        # 2. Max Pooling
        input_mask_expanded = attention_mask.unsqueeze(-1).expand(token_embeddings.size()).float()
        token_embeddings[input_mask_expanded == 0] = -1e9
        max_pool = torch.max(token_embeddings, 1)[0]  # [batch, 480]
        pool_vectors.append(max_pool)
    
        # 3. Mean Pooling
        sum_embeddings = torch.sum(token_embeddings * input_mask_expanded, 1)
        sum_mask = input_mask_expanded.sum(1).clamp(min=1e-9)
        mean_pool = sum_embeddings / sum_mask  # [batch, 480]
        pool_vectors.append(mean_pool)
    
        # 4. Sqrt Mean Pooling
        sqrt_mean_pool = sum_embeddings / torch.sqrt(sum_mask)  # [batch, 480]

...
class Boundary_Completeness_Regressor(torch.nn.Module):
    ...
    def forward(self, feat, proposals, mask, pseudo_labels=None, iou_thresh=0.5, is_training=True):
        ...
        feat_fuse = self.prop_fusion(iou_feat)
        feat_fuse = feat_fuse.transpose(-1, -2)
        prop_iou = self.prop_completeness(feat_fuse).squeeze(1)
        iou_pred = self.sigmoid(prop_iou)

        feat1 = feat1.transpose(-1, -2)
        feat3 = feat3.transpose(-1, -2)
        start_reg = self.start_reg_conv(feat1).squeeze(1)
        end_reg = self.end_reg_conv(feat3).squeeze(1)
    ...
    def segments_iou(self, segments1, segments2):
        ...
        tt1 = torch.maximum(segments1[..., 0], segments2[..., 0])
        tt2 = torch.minimum(segments1[..., 1], segments2[..., 1])
...
class I_Model(torch.nn.Module):
    ...
    def forward(self, features, proposals, pseudo_labels=None, is_training=True):
        ...
        outputs = {
                'prop_refined': prop_refined,
                'iou_pred_orig': iou_pred_orig.unsqueeze(-1),
                'iou_pred_refined': iou_pred_refined.unsqueeze(-1),
                'prop_mask': prop_mask,
            }
        if is_training:
    ...
    def criterion(self, outputs, args):
        ...
        prop_mask = prop_mask.unsqueeze(2).bool()
    ...
    def extract_roi_features(self, features, proposals, is_training):
        ...
        for i in range(batch):
            ...
            fill_len = torch.ceil(0.25 * len_prop.max()).long() + 1
            ...
            feature = feature.transpose(0, 1).unsqueeze(0).unsqueeze(3)
            feat_roi = torchvision.ops.roi_align(feature, [boxes], [self.roi_size, 1])
            feat_roi = feat_roi.squeeze(3).transpose(1, 2)
            prop_features[i, :proposal.shape[0], :, :] = feat_roi
            prop_mask[i, :proposal.shape[0]] = 1

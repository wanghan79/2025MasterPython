...
class EsmPPIModel(pl.LightningModule):
    ...
    def __build_model(self) -> None:
        ...
        self.classification_head = nn.Sequential(OrderedDict([
            ...
            ("dense1", nn.Linear(self.adapted_dim, int(self.adapted_dim / 16))),  # 2048->128
            ("dropout2", nn.Dropout(0.2)),
            ("dense2", nn.Linear(int(self.adapted_dim  / 16), int(self.adapted_dim  / (16*16)))),  # 128->8
            ("dropout3", nn.Dropout(0.2)),
            ("dense3", nn.Linear(int(self.adapted_dim  / (16*16)), 1)),  # 8->1
        ]))
    ...
    def pool_strategy(self, features):
        """修改后的池化策略，包含维度适配"""
        token_embeddings = features['token_embeddings']  # [batch, seq_len, 640]

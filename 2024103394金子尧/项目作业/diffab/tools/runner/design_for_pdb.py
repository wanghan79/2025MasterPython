...
def create_data_variants(config, structure_factory):
    ...
    data_variants = []
    if config.mode.startswith('single_cdr'):
...
def design_for_pdb(args):
    ...
    # Override config parameters with CLI args 
    if args.num_samples  is not None:
        config.sampling.num_samples  = args.num_samples  
    config.sampling.sample_step_mode  = args.sample_step_mode  
    config.sampling.sample_step_num  = args.sample_step_num  
    config.sampling.sample_step_period  = args.sample_step_period  
    ...
    if args.no_renumber:
        ...
        # 修改2：把编号的文件存放在其他路径下
        # out_pdb_path = os.path.splitext(in_pdb_path)[0] + '_chothia.pdb'#分离文件名和扩展名的函数
        chothia_dir=os.path.dirname(in_pdb_path)+"/chothia"  #新建文件夹
        if not os.path.exists(chothia_dir):
            os.makedirs(chothia_dir)
        out_pdb_path=os.path.dirname(in_pdb_path)+"/chothia/"+os.path.splitext(data_id)[0]+'_chothia.pdb'
    ...
    log_dir = get_new_log_dir(
        ...
        # prefix=data_id
        prefix=os.path.splitext(data_id)[0]
    )
    ...
    cfg_ckpt.model.diffusion["prior_b"] = args.prior_b # add weight of prior aa distributions
    ...
    for variant in data_variants:
        ...
        for batch in tqdm(loader, desc=variant['name'], dynamic_ncols=True):
            ...
            elif ('ddg' in config.mode) or ('hydro' in config.mode):
                # De novo design with sampling in sampling (ddG, hydropathy, or both)
                traj_batch = model.sample(batch, sample_opt={
                    'pbar': True,
                    'sample_structure': config.sampling.sample_structure,
                    'sample_sequence': config.sampling.sample_sequence,
                    'sample_step_by_ddg': config.sampling.sample_step_by_ddg,
                    'sample_step_by_hydro': config.sampling.sample_step_by_hydro,
                    'sample_step_mode': config.sampling.sample_step_mode,
                    'sample_step_num': config.sampling.sample_step_num,
                    'sample_step_period': config.sampling.sample_step_period,
                })
            else:
                # De novo design
                traj_batch = model.sample(batch, sample_opt={
            ...
            if args.save_traj:
                ...
                all_traj_batch.append(traj_batch)
                continue
        ...
        if args.save_traj:
            with open(os.path.join(log_dir, variant["tag"], "traj_data.pkl"), "wb") as f:
                pickle.dump({
                    "data_native": data_native,
                    "data_cropped": data_cropped,
                    "variant": variant,
                    "batch": all_batch,
                    "traj_batch": all_traj_batch,
                }, f)
...
def args_from_cmdline():
    ...
    parser.add_argument('--num_samples',  type=int, default=None, 
                      help='Number of samples to generate (覆盖yml中的num_samples)')
    ...
    parser.add_argument('--prior_b',  type=float, default=0)
    parser.add_argument('--save_traj',  action='store_true', default=False)
    parser.add_argument('--sample_step_mode',  type=str, default='min')
    parser.add_argument('--sample_step_num',  type=int, default=20)
    parser.add_argument('--sample_step_period',  type=int, default=1)

...
class Encoder(nn.Module):
    ...
    def forward(self, input_features, prototypes=None):
        ...
        prototypes = prototypes.to(input_features.device)
        prototypes = prototypes.view(1,F,-1).expand(B,-1,-1)
        if hasattr(self, 'RAB'):
            layer_features = torch.cat([input_features, prototypes], dim=2)
            for layer in self.RAB:
                layer_features, _ = layer(layer_features)
            input_features = layer_features[:, :, :T]
        embeded_features = self.feature_embedding(input_features)
...
class S_Model(nn.Module):
    ...
    def forward(self, input_features, vid_labels=None):
        ...
        cas = self.classifier(embeded_feature)
        cas = cas.permute(0, 2, 1)
        cas = self.sigmoid(cas)
        ...
        cas_P = cas_S * (1 - bkg_score.unsqueeze(2))
        cas_fuse = torch.cat((cas_P, bkg_score.unsqueeze(2)), dim=2)
